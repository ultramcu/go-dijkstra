...
//
// Goroutine safety: an StGraph instance is NOT safe for concurrent
// use during a search; the search mutates per-vertex state on the
// graph itself.
package dijkstra
...
// search and optional vertex blocking.
type StGraph struct {
	vertex        []StVertex
	debugEn       bool
...
func (g *StGraph) VertexBLockClear() { g.blockedVertex = nil }
...
func (g *StGraph) VertexBLock(name string) bool {
	if g.VertexIsBLock(name) {
		return false
	}
	if !g.VertexIsExist(name) {
		return false
	}
	g.blockedVertex = append(g.blockedVertex, name)
	return true
...
// false if name was present in neither case (i.e. nothing was blocked
...
func (g *StGraph) Debug(debugEn bool) { g.debugEn = debugEn }
...
func (g *StGraph) VertexLength() int { return len(g.vertex) }
...
func (g *StGraph) VertexAdd(name string, x, y, heading float64, edges ...StEdge) bool {
	...
	for _, e := range edges {
		if g.VertexIsBLock(e.ToVertexName) {
...
// getAdjacencyVertices returns the slice of edges leaving `name`.
// Returns nil if the vertex doesn't exist.
func (g *StGraph) getAdjacencyVertices(name string) []StEdge {
	i := g.VertexFind(name)
	if i < 0 {
		return nil
	}
